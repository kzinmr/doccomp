...
<body>
  ...
  <form on:submit|preventDefault={handleSubmit}>
    ...
    <h3>Pattern:</h3>
    <p>
      <input bind:value={pattern} size="32" placeholder="enter pattern to match" /><span
        >{maxlengthspan}</span
      ><br />
    ...
    <p>
      ...
      ファジィ位置（上記で指定）にどの程度近づけなければならないかを決定します。 ファジィ位置から `distance`
      文字ぶん離れた文字が完全に一致した場合は、完全な不一致としてスコア付けされます。 距離'0'の場合、指定した位置に正確に一致する必要があり、
    ...
    <p>
      ...
      マッチアルゴリズムはどの時点であきらめるのか。 しきい値「0.0」は（文字と場所の両方が）完全に一致することを要求し、
      しきい値「1.0」は何でも一致する。
    </p>
